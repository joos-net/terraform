...
Для уникальности в рамках одной стреды все ресурсы должны определяться ресурсом
и именем - имени тут не хватает

Invalid resource name - A name must start with a letter or underscore and may contain
only letters, digits, underscores, and dashes.
Не правильное имя - имя должно начинаться с буквы или подчеркивания.

Reference to undeclared resource - A managed resource "random_password" "random_string_FAKE"
